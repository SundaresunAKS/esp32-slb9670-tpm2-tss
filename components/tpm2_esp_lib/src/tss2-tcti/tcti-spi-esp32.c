...
TSS2_RC platform_spi_transfer (void* user_data, const void *data_out, void *data_in, size_t cnt)
{
    ...
    printf("platform_spi_transfer Called.......\n");
    ...
    if (ret != ESP_OK) {
    	printf("Error:: spi_device_polling_transmit \n");
...
TSS2_TCTI_CONTEXT* create_tcti_ctx()
{
    ...
    spi_device_interface_config_t dev_cfg = {
        .clock_speed_hz = 10*1000*1000, // 1MHz, but tested up to 22MHz
        .mode = 0,
        .spics_io_num = -1,
        .queue_size = 1,
        .pre_cb = NULL,
...
//    gpio_pad_select_gpio(PIN_NUM_RST);
    ...
    if (rc != TSS2_RC_SUCCESS) {
        printf("Failed to get allocation size for device tcti context: 0x%lx\n", rc);
        return NULL;
    }
    ...
    if (rc != TSS2_RC_SUCCESS) {
        printf("Failed to initialize device TCTI context: 0x%lx\n", rc);
        free(tcti_ctx);
        return NULL;
    }
